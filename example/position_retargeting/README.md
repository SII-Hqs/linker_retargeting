...
python detect_from_rgbd.py   --robot-name linker_l20   --rgb-dir ./my_data/rgb   --depth-dir ./my_data/depth   --output-path ./output/retarget.pkl   --visualize   --visualization-output ./output/debug.mp4   --save-visualization-frames

python detect_from_rgbd.py   --robot-name linker_l20   --rgb-dir ./test_p/rgb  --depth-dir ./test_p/depth  --output-path ./output/retarget_test.pkl --visualize   \
  --visualization-output ./output/debug.mp4   \
  --save-visualization-frames

python plot_grouped_qpos_curves.py \
    output/result.pickle \
    --save output/grouped_qpos_curves.png \
    --unit deg


cd /home/maker01/Code/dex-retargeting

python export_dexycb_retarget_pickle.py \
  --dexycb-dir dexYCB \
  --output-path output/dexycb_linker_l20.pkl \
  --robot-name linker_l20 \
  --data-id 4 \
  --fps 10


python visualize_hand_object.py --dexycb-dir=dexYCB --robots linker_l20 shadow

cd /home/maker01/Code/dex-retargeting/example/position_retargeting

python export_dexycb_retarget_pickle.py \
  --dexycb-dir dexYCB \
  --output-path output/dexycb_shadow.pkl \
  --robot-name shadow \
  --data-id 4 \
  --fps 10
