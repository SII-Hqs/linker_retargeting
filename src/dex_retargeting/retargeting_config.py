...
class RetargetingConfig:
    ...
    vector_scaling_factors: Optional[np.ndarray] = None
    ...
    def __post_init__(self):
        ...
        if self.type == "vector":
            ...
            if self.vector_scaling_factors is not None:
                self.vector_scaling_factors = self.vector_scaling_factors.squeeze()
                if self.vector_scaling_factors.shape != (
                    len(self.target_origin_link_names),
                ):
                    raise ValueError(
                        "Vector retargeting scaling factors and link names dim mismatch"
                    )
    ...
    def from_dict(cls, cfg: Dict[str, Any], override: Optional[Dict] = None):
        ...
        if "vector_scaling_factors" in cfg:
            cfg["vector_scaling_factors"] = np.array(cfg["vector_scaling_factors"])
