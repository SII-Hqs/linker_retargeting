...
class VectorOptimizer(Optimizer):
    ...
    def __init__(
        ...
        elif scaling.shape == (len(target_origin_link_names),):
            self.scaling = scaling[:, None]
        else:
            raise ValueError(
                "Vector scaling must be a scalar or one value per target vector."
            )
    ...
    def get_objective_function(
        ...
        qpos = np.zeros(self.num_joints)
        qpos[self.idx_pin2fixed] = fixed_qpos
        torch_target_vec = torch.as_tensor(target_vector) * torch.as_tensor(
            self.scaling
        )
